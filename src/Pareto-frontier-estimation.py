# from __future__ import division
import math
import pandas as pd
import random
import pygmo
import re


### Useful functions
def format_number(primitives):
    # format [morphosyntax, denotation, complexity]
    primitives_formatted = []
    for prim in primitives:
        i_formatted = [str(prim), prim, 1]
        primitives_formatted.append(i_formatted)
    return primitives_formatted


def delete_multiple_element(list_object, indices):
    for ind in sorted(indices, reverse=True):
        del list_object[ind]
    return list_object


def filter_within(constructions):
    if len(constructions) == 0:
        return constructions
    elif len(constructions) > 0:
        unwanted = []
        for i in range(len(constructions)):
            for j in range(len(constructions)):
                if i == j:
                    continue
                else:
                    if constructions[i][1] == constructions[j][1]:
                        if (constructions[i][2] == constructions[j][2] and i < j) or (
                                constructions[j][2] > constructions[i][2]):  # throw out the one with larger complexity; if they are of equal complexity, throw out the one with larger index
                            unwanted.append(j)
        unwanted = list(set(unwanted))
        if len(unwanted) > 0:
            constructions = delete_multiple_element(constructions, unwanted)
        return constructions


# Filter_across
def filter_across(constructions, lower_constructions):
    if len(constructions) == 0:
        return constructions
    elif len(constructions) > 0:
        unwanted = []
        for i in range(len(constructions)):
            for j in range(len(lower_constructions)):
                if constructions[i][1] == lower_constructions[j][1]:
                    unwanted.append(i)
        unwanted = list(set(unwanted))
        if 0 < len(unwanted) < len(constructions):
            constructions = delete_multiple_element(constructions, unwanted)
        return constructions


# Filter 100
def filter_100(constructions):
    unwanted = []
    for i in range(len(constructions)):
        if constructions[i][1] > 99:
            unwanted.append(i)
        unwanted = list(set(unwanted))
    if 0 < len(unwanted) < len(constructions):
        constructions = delete_multiple_element(constructions, unwanted)
    return constructions


# Expected morphosyntactic complexity
# Prior
prior_sum = 0  # for normalization of prior probas
for i in range(1, 100, 1):
    prior_sum += i ** (-2)


def probaf(number):
    return (number ** (-2)) / prior_sum

def expected_complexity(language_id, df):
    temp = df[df['language'] == language_id]
    return sum(temp['weighted_complexity'])


# Remove takes a language df, and removes one row (one item) from the language if l has more than 1 item
def remove(language):
    pos = random.choice([0, 1])
    if len(language[pos]) > 1:  # only remove if the digits/multis have more than one word
        remove_index = random.randint(0, len(language[pos]) - 1)
        del language[pos][remove_index]
    return language


# Add an item. But, only add a multiplicative if there is less than 5 multiplicatives!
def add(language):
    pos = random.choice([0, 1])
    if len(language[0] + language[1]) < 99:  # only add if not all words are there
        if pos == 0:
            add_value = random.choice([i for i in range(1, 100) if i not in language[0] + language[
                1]])  # add only if the word not already in the language
            language[pos].append(add_value)
        if pos == 1:
            if len(language[pos]) < 5:
                add_value = random.choice([i for i in range(1, 100) if i not in language[0] + language[
                    1]])  # add only if the word not already in the language
                language[pos].append(add_value)
    return language

# Interchange an item in the language
def interchange(language):
    return remove(add(language))


# Define what it means to mutate a language
def mutate(language):
    possible_mutations = [interchange]
    if len(language[0] + language[1]) < 99:
        possible_mutations.append(add)
    if len(language[0] + language[1]) > 1:
        possible_mutations.append(remove)
    mutation = random.choice(possible_mutations)
    return mutation(language)

# Create offsprings of a list of languages
def sample_mutated(languages_list, amount, max_mutations):
    amount -= len(languages_list)  # this is so that the mutations are added up to the original languages so that their sum = amount
    amount_per_language = int(math.floor(amount / len(languages_list)))
    amount_random = amount % len(languages_list)

    mutated_languages = []

    for lang in languages_list:
        for i in range(amount_per_language):
            num_mutations = random.randint(1, max_mutations)
            mutated_language = [[x for x in l] for l in lang]
            print(mutated_language)
            for j in range(num_mutations):
                mutated_language = mutate(mutated_language)

            mutated_languages.append(mutated_language)

    for i in range(amount_random):
        language_list_2 = [[[x for x in l] for l in lang] for lang in languages_list]
        language = random.choice(language_list_2)
        mutated_language = mutate(language)
        mutated_languages.append(mutated_language)


    return mutated_languages


# Generate a single language from its morphemes, arguments a list of digits and a list of multiplicatives
def generate(digits, multiplicatives):
    np = {}
    np['phrase1'] = format_number(multiplicatives)
    np['number1'] = format_number(digits) + format_number(multiplicatives)
    np['number1'] = filter_within(np['number1'])
    lowest_complexity = np['number1']
    start = 2
    while start < 8 and len(lowest_complexity) != 200:
        # first we build phrases with * and append them to NumP
        np['phrase' + str(start)] = []
        np['number' + str(start)] = []
        for first_element in np['number' + str(start - 1)]:
            for second_element in np['phrase1']:
                current_multiplication = [first_element[0] + '*' + second_element[0],
                                          first_element[1] * second_element[1],
                                          first_element[2] + second_element[2] + 1]
                if current_multiplication[1] < 201:  # 101 so that things like 5*20-1 can be generated
                    np["phrase" + str(start)].append(current_multiplication)
        np['number' + str(start)].extend(np["phrase" + str(start)])
        # second we build NumP with + and -
        for x in range(1, start, 1):
            for y in range(1, start, 1):
                if x + y == start:
                    for first_element in np['phrase' + str(x)]:
                        for second_element in np['number' + str(y)]:
                            current_plus = [first_element[0] + '+' + second_element[0],
                                            first_element[1] + second_element[1],
                                            first_element[2] + second_element[2] + 1]
                            current_minus = [first_element[0] + '-' + second_element[0],
                                             first_element[1] - second_element[1],
                                             first_element[2] + second_element[2] + 1]
                            if current_plus[1] < 201:
                                np["number" + str(start)].append(current_plus)
                            if 0 < current_minus[1] < 201:
                                np["number" + str(start)].append(current_minus)

        np["number" + str(start)] = filter_within(np["number" + str(start)])
        np["number" + str(start)] = filter_across(np["number" + str(start)], lowest_complexity)
        lowest_complexity.extend(np["number" + str(start)])
        lowest_complexity = filter_within(lowest_complexity)

        start = start + 1
    lowest_complexity = filter_100(lowest_complexity)
    if len(lowest_complexity) == 99:
        return lowest_complexity
    else:
        return []


# Generate languages from a list of languages
def generate_list(language_list, generation):
    all_languages_df = pd.DataFrame(
        columns=['language', 'morphology', 'extension', 'digs', 'multis', 'complexity', 'word_n', 'weighted_complexity',
                 'expected_complexity', "coordinates"])
    for language in language_list:
        language_name = 'language' + 'gen' + str(generation) + 'index' + str(
            language_list.index(language)) + '_' + '.'.join([str(elem) for elem in language[0]]) + '_' + '.'.join(
            [str(elem) for elem in language[1]])
        language_expressions = generate(language[0], language[1])
        if len(language_expressions) > 0:
            language_expressions_df = pd.DataFrame(language_expressions,
                                                   columns=["morphology", "extension", "complexity"])
            language_expressions_df['language'] = language_name
            language_expressions_df['digs'] = '.'.join([str(elem) for elem in language[0]])
            language_expressions_df['multis'] = '.'.join([str(elem) for elem in language[1]])
            language_expressions_df['word_n'] = len(language[0]) + len(language[1])
            language_expressions_df['weighted_complexity'] = language_expressions_df['extension'].map(probaf) * language_expressions_df['complexity']
            language_expressions_df['expected_complexity'] = expected_complexity(language_name, language_expressions_df)
            all_languages_df = pd.concat([all_languages_df, language_expressions_df])
            all_languages_df['coordinates'] = list(zip(all_languages_df.word_n, all_languages_df.expected_complexity))
            all_languages_df.reset_index(inplace=True, drop=True)
    all_languages_df.to_csv(Folder + "evo_full_generation_" + str(generation) + ".csv", index=False)
    # prepare output where 1 row = 1 language
    all_languages_output = all_languages_df[['language', 'digs', 'multis', 'word_n', 'expected_complexity', 'coordinates']]
    all_languages_output = all_languages_output.drop_duplicates()
    all_languages_output.reset_index(inplace=True, drop=True)
    all_languages_output.to_csv(Folder + "evo_coordinates_generation_" + str(generation) + ".csv", index=False)
    return all_languages_output


######################################
# Evolutionary algorithm
######################################
if __name__ == "__main__":

    # Read relevant files
    Folder = "../data/"
    # Generate generation 0 that should have 2000 languages
    languages = []
    for i in range(2000):
        n_multi = random.choice(range(1, 6))  # max 5 n_multi, and thus max 98 n_digs
        n_dig = random.choice(range(1, 100 - n_multi))
        multis = random.sample(range(1, 100), n_multi)
        digs = random.sample([i for i in range(1, 100) if i not in multis], n_dig)
        current_language = [digs, multis]
        languages.append(current_language)

    # Run the evolutionary algorithm for 100 generations, each with 2000 languages
    num_of_gen = 100
    sample_size = 2000
    max_mut = 3

    for i in range(0, num_of_gen + 1):
        if i == 0:
            generated_final = generate_list(languages, i)
            generated_languages = generate_list(languages, i)
            generated_final = pd.concat([dominant_lang_df, generated_languages])
        generated_final.reset_index(inplace=True, drop=True)
        dominating_indices = pygmo.non_dominated_front_2d(points=list(generated_final['coordinates']))
        dominant_lang_df = generated_final.iloc[dominating_indices].copy()
        dominant_lang_df.reset_index(inplace=True, drop=True)
        dominant_lang_df.to_csv(r'' + Folder + 'gen ' + str(i) + 'dominant.csv', index=False)
        dominant_languages_list = []
        for index, row in dominant_lang_df.iterrows():
            dominant_lang = [list(map(int, re.findall('\\d+', row['digs']))),
                             list(map(int, re.findall('\\d+', row['multis'])))]
            dominant_languages_list.append(dominant_lang)
        languages = sample_mutated(dominant_languages_list, sample_size, max_mut)

    # Find the dominant languages from the final generation + natural languages
    natural_languages =  pd.read_csv(Folder + "natural_lang_complexity_measures.csv")
    natural_languages['coordinates'] = list(zip(natural_languages.word_n, natural_languages.expected_complexity))
    print(natural_languages)
    final_generation_dominant_lang = dominant_lang_df
    final_generation_dominant_lang = final_generation_dominant_lang.drop(columns = ['digs','multis'])
    domselect = pd.concat([natural_languages, final_generation_dominant_lang])
    domselect.reset_index(inplace=True, drop=True)
    print(domselect)
    dominating_indices = pygmo.non_dominated_front_2d(points = list(domselect['coordinates']))
    dominant_lang_df = domselect.iloc[dominating_indices]
    dominant_lang_df.to_csv(r''+Folder+'pareto_dominant.csv', index=False)